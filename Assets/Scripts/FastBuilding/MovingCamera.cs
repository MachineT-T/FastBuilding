...
public class MovingCamera : MonoBehaviour
{
    ...
    private int MouseZoomMin = 0; //相机距离最小值
    ...
    private int xMinLimit = -360;
    private int xMaxLimit = 360;
    ...
    void Start()
    {
        //这里就是设置一下初始的相机视角以及一些其他变量
        var angles = transform.eulerAngles;
        x = angles.x;
        y = angles.y;
        CameraTargetPosition = target.position;
        storeRotation = Quaternion.Euler(x + 60, y, 0);
        transform.rotation = storeRotation; //设置相机姿态，让镜头向下倾斜60°
    ...
    void Update()
    {
        ...
        {
            y += Input.GetAxis("Mouse X") * ySpeed * 0.02f;//鼠标在X轴上移动时，视角绕Y轴旋转
            x -= Input.GetAxis("Mouse Y") * xSpeed * 0.02f;//鼠标在Y轴上移动时，视角绕X轴旋转
